...
## Automated Pre-Deploy Scanner

???? deploy ????????????? user data � ??? pdpa-guard ????? scan ?????????:
...
???? 4 ????????? AI ??????????:
- **Cloud Region** � Firebase/Supabase ???????? region ??? (us-central1 = cross-border transfer ?????)
- **Security Rules** � llow read, write: if true ????????????? production
- **PII Fields** � grep ?? field ????, ?????, ???????????, ???????, ????? (???+??????)
- **Subprocessors** � SDK ???????????????????????? (Sentry, Gemini, LINE API, Omise ???)

???????????????? pdpa-report.md ????? checklist ??????????

????? CI/pre-deploy hook:
```bash
# package.json
"scripts": { "predeploy": "npx pdpa-guard . --fail-on critical" }
```

package: https://npmjs.com/package/pdpa-guard
